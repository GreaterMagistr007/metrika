if [ "$(docker ps -q -f name=project_server)" ]; then
    ...
    # Ставим пакеты
    docker exec -it project_server bash -c "cd /home/user/services/metrika.loc && composer install && composer update"
    # Права на запись
    docker exec -it project_server bash -c "chmod 777 /home/user/services/metrika.loc/storage -R"
    # Перегенерация ключа
    docker exec -it project_server bash -c "cd /home/user/services/metrika.loc/ && php artisan key:generate"
